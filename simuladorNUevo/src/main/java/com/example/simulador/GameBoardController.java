...
public class GameBoardController {
    ...
    private ListaSimple<Recurso> recursosActivos = new ListaSimple<>();
    ...
    private void mostrarInformacionCelda(int row, int col) {
        ...
        for (int i = 0; i < individuos.getNum_elementos(); i++) {
        ...
        for (int i = 0; i < recursosActivos.getNum_elementos(); i++) {
            Recurso recurso = (Recurso) recursosActivos.getElemento(i).getData();
    ...
    private void mostrarIndividuos() {
        ...
        for (int i = 0; i < individuos.getNum_elementos(); i++) {
            Individuo individuo = (Individuo) individuos.getElemento(i).getData();
    ...
    private Recurso seleccionarTipoRecursoAleatorio() {
        ...
        ListaSimple<Recurso> tiposRecursosDisponibles = new ListaSimple<>();
        ...
        Random random = new Random();
        int indiceTipoRecurso = random.nextInt(tiposRecursosDisponibles.getNum_elementos());
        return (Recurso) tiposRecursosDisponibles.get(indiceTipoRecurso);
    }
    ...
    private void controlarDuracionRecursos() {
        ...
        ListaSimple<Recurso> recursosAEliminar = new ListaSimple<>(recursosActivos.getNum_elementos());

        // Iterar sobre la lista de recursos activos
        for (int i = 0; i < recursosActivos.getNum_elementos(); i++) {
            Recurso recurso = (Recurso) recursosActivos.get(i);
        ...
        for (int i = 0; i < recursosAEliminar.getNum_elementos(); i++) {
            Recurso recurso = (Recurso) recursosAEliminar.get(i);
    ...
    private Recurso encontrarRecursoAleatorio() {
        ...
        if (recursosActivos.isVacia()) {
            return null;
        } else {
            Random random = new Random();
            int index = random.nextInt(recursosActivos.getNum_elementos());
            return (Recurso) recursosActivos.get(index);
        }
    }
    ...
    private void moverIndividuosAutomaticamente() {
        ...
        ListaSimple<Individuo> copiaIndividuos = new ListaSimple<>(individuos.getNum_elementos());

        // Copiar los elementos de la lista original a la copia
        for (int i = 0; i < individuos.getNum_elementos(); i++) {
            Individuo individuo = (Individuo) individuos.getElemento(i).getData();
            copiaIndividuos.add(individuo);
        }

        // Iterar sobre la copia
        for (int i = 0; i < copiaIndividuos.getNum_elementos(); i++) {
            Individuo individuo = (Individuo) copiaIndividuos.getElemento(i).getData();
            ...
            if (individuo.getTurnosDeVida() > 0) {
                ...
                // si ha alcanzado el fin de turnos de vida, eliminado
    ...
    private Button guardarJuegoButton() {
        ...
        saveButton.setOnAction(e -> {
            try {
                int posicionX = EstadoJuego.getPosicionX();
                int posicionY = EstadoJuego.getPosicionY();
                int puntaje = EstadoJuego.getScore();

                EstadoJuego miEstadoJuego = new EstadoJuego(posicionX, posicionY, puntaje);
                // Aquí deberías pasar una instancia de EstadoJuego válida, por ejemplo, "miEstadoJuego"
                GameData.saveGame(miEstadoJuego, "EstadoJuego.ser");
        ...
        loadButton.setOnAction(e -> {
            try {
                EstadoJuego gamestate = GameData.loadGame("EstadoJuego.ser");
                // Actualiza el juego con el estado cargado
                updateGameWithLoadedState(gamestate);
            } catch (IOException ex) {
                ex.printStackTrace();
            }
        });
    ...
    private void updateGameWithLoadedState(EstadoJuego gamestate) {
        // Implementa la lógica para actualizar el juego con el estado cargado
        System.out.println("Posición del jugador: (" + gamestate.getPosicionX() + ", " + gamestate.getPosicionY() + ")");
        System.out.println("Puntuación: " + gamestate.getScore());
    }

//"EstadoJuego.ser" es el nombre del archivo donde se guardará o desde donde se cargará el estado del juego
//.ser es una convención usada para indicar que un archivo contiene datos serializados. No es obligatorio usar .ser, pero ayuda a identificar rápidamente el propósito del archivo.
